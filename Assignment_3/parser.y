...
%type <astree> translation_unit external_declaration shift_expression relational_expression init_declarator_list
...
declaration
	...
	| declaration_specifiers init_declarator_list ';'				{ $$ = newast("DECL", $1, $2); if(debug)printf(" --declaration : declaration_specifiers init_declarator_list ';'\n"); }
	;
...
init_declarator
	: declarator												{ $$ = $1; if(debug)printf(" --init_declarator : declarator\n"); }
	| declarator '=' initializer				{ $$ = newast("=", $1, $3); if(debug)printf(" --init_declarator : declarator '=' initializer\n"); }
	;
...
direct_declarator
	...
	| '(' declarator ')'															{ $$ = $2; }
	| direct_declarator '[' constant_expression ']'   { $$ = newast("[]", $1, $3); }
	| direct_declarator '[' ']'
...
declaration_list
	: declaration																	{ $$ = $1; if(debug)printf(" --declaration_list : declaration\n"); }
	| declaration_list declaration								{ $$ = newast("DECL", $1, $2); if(debug)printf(" --declaration_list : declaration_list declaration\n"); }
	;
